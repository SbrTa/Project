...
if(isset($_POST['submit']))
{
	...
	$station_Name=test_input($_POST['station_Name']);
    $arrival_time=test_input($_POST['arrival_time']);
    $departure_time=test_input($_POST['departure_time']);
 
 $statement = $db->prepare("INSERT INTO routes (train_id,stop_number,station_id,station_Name,arrival_time,departure_time)VALUES(?,?,?,?,?,?)");
		
	$statement->execute(array($train_id,$stop_number,$station_id,$station_Name,$arrival_time,$departure_time));
...
function validate_from()
			{
				...
				x=document.forms["adform"]["station_Name"].value;
				if(x==null || x=="")
				{
					alert("Enter Station Station_Name");
					return false;
				}
				...
				<form class="form-horizontal" name="adform" action="" onsubmit="return validate_from()" method="post">
				 ...
				 <div class="form-group">
				  ...
				  <div class="form-group">
					<label for="inputPassword3" class="col-sm-2 control-label">Station_Name</label>
					<div class="col-sm-8">
					  <input type="text" class="form-control" id="inputPassword3" name="station_Name" placeholder="Station_Name">
					</div>
				  </div>
				 <div class="form-group">
