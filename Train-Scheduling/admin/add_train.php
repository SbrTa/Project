...
if(isset($_POST['submit']))
{
    ...
    $destination_id=test_input($_POST['destination_id']);
     ...
     //mysqli_query($con,"INSERT INTO `trains` (`train_id`,`train_name`,`train_type`,`source_stn`,`destination_stn`,`source_id`,`destination_id`) VALUES ('$train_id','$train_name','$train_type','$source_stn','$destination_stn','$source_id','$destination_id')") or die(mysql_error());
     //header("location:adminhome.php");
	 
 $statement = $db->prepare("INSERT INTO trains (train_id,train_name,train_type,source_stn,destination_stn,source_id,destination_id) VALUES (?,?,?,?,?,?,?)");
	//$statement->execute($db);	
  $statement->execute(array($train_id,$train_name,$train_type,$source_stn,$destination_stn,$source_id,$destination_id));
	header("location:adminhome.php");
}
?>
